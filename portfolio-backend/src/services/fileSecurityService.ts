...
export class FileSecurityService {
  ...
  private readonly ALLOWED_EXTENSIONS = ['.pdf', '.docx', '.doc', '.txt'];
  ...
  async scanFile(filePath: string, mimeType: string): Promise<SecurityScanResult> {
    ...
    try {
      ...
      console.log(`🔒 Security scan complete for ${path.basename(filePath)}: ${isSecure ? 'SECURE' : 'THREATS DETECTED'}`);
      if (threats.length > 0) {
  ...
  private validateBasicFileProperties(filePath: string, mimeType: string): { isValid: boolean; threats: string[] } {
    const threats: string[] = [];
  ...
  private async validateFileSignature(filePath: string, mimeType: string): Promise<{ isValid: boolean; threats: string[] }> {
    const threats: string[] = [];
  ...
  private async scanFileContent(filePath: string): Promise<{ isClean: boolean; threats: string[] }> {
    const threats: string[] = [];
    ...
    try {
      ...
      if (content.includes('\0') && path.extname(filePath).toLowerCase() === '.txt') {
  ...
  quarantineFile(filePath: string): string {
    ...
    const quarantinePath = path.join(quarantineDir, `quarantined_${timestamp}_${path.basename(filePath)}`);
